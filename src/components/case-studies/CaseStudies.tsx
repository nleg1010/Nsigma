...
const CaseStudies = ({ data }: any) => {
     ...
     return (
          <section className='pb-20'>
               ...
               <section className='container relative mx-auto px-8 md:px-0' id="casestudy">
                    <Slider ref={slider} {...settings}>
                         {
                              data?.map((item: any, idx: number) => (
                                   <div className='p-3' key={idx}>
                                        <CaseStudiesCard data={item} />
                                   </div>
                              ))
                         }
                    </Slider>
                    <div className='text-white text-4xl'>
                         <button className='absolute top-1/2 -left-0 md:-left-7 transform -translate-y-1/2' onClick={() => slider?.current?.slickPrev()}>
                              <IoIosArrowBack />
                         </button>
                         <button className='absolute top-1/2 -right-0 md:-right-7 transform -translate-y-1/2' onClick={() => slider?.current?.slickNext()}>
                              <IoIosArrowForward />
                         </button>
                    </div>
               </section>
